...
if __name__ == "__main__":
    ...
    ##########
    # T_model = PointRCNN(num_classes = train_loader.dataset.num_class, use_xyz = True, mode = 'TEST')
    ...
    # if args.ckpt is not None:
    #     T_pure_model = T_model.module if isinstance(T_model, torch.nn.DataParallel) else T_model
    #     T_it, T_start_epoch = train_utils.load_checkpoint(T_pure_model, optimizer, filename = '/home/hust/EPNet-ori/tools/log/Car/full_epnet_without_iou_branch/ckpt_tmp/checkpoint_epoch_50.pth', logger = logger)
    #     # last_epoch = start_epoch + 1    
    #     T_model.eval()
    ##########
    ...
    sample_loss_list = trainer.train(
            # T_model,
            it,
            start_epoch,
            args.epochs,
    ...
    # plt.figure()
    # plt.plot(sample_loss_list,'b',label = 'pedestrain loss')
    # plt.ylabel('sample_loss')
    # plt.xlabel('iter_num')
    # plt.legend()
