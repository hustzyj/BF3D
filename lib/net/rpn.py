...
class RPN(nn.Module):
    ...
    def forward(self, input_data):
        ...
        if cfg.LI_FUSION.ENABLED:
            ...
            if 'rpn_cls_label' in input_data.keys():
                rpn_cls_label = input_data['rpn_cls_label']
            else :
                rpn_cls_label = None
            backbone_xyz, backbone_features,sa_ins_preds, sample_idx  = self.backbone_net(pts_input, img_input, xy_input, rpn_cls_label)  # (B, N, 3), (B, C, N)
        else:
        ...
        ret_dict = { 'rpn_cls'     : rpn_cls, 'rpn_reg': rpn_reg,
                     'backbone_xyz': backbone_xyz, 'backbone_features': backbone_features,'sample_cls':sa_ins_preds,'sample_idx': sample_idx }
