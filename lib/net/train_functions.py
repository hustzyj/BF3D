...
def model_joint_fn_decorator():
    ...
    def model_fn(model, data):
        ...
        if cfg.RPN.ENABLED:
            ...
            if not cfg.RCNN.ROI_SAMPLE_JIT:
                ...
                input_data['rpn_cls_label'] = rpn_cls_label
        # input()
        ...
        if cfg.RPN.ENABLED and not cfg.RPN.FIXED:
            ...
            sample_cls = ret_dict['sample_cls']
            sample_idx = ret_dict['sample_idx']
           
            sample_loss = get_sample_loss(model,sample_cls,sample_idx,rpn_cls_label,tb_dict)
            if sample_loss!=0:
                loss += sample_loss
                disp_dict['sample_loss'] = sample_loss.item()
            ...
            ######################
            # S_rpn_cls = rpn_cls
            # T_rpn_cls = T_ret_dict['rpn_cls']
            # rpn_loss_cls_KD = get_rpn_loss_KD(model, S_rpn_cls, T_rpn_cls, tb_dict, T)
            # loss += rpn_loss_cls_KD * 100

            # s_rpn_cls = torch.sigmoid(ret_dict['rpn_cls'].view(-1)/T).log()      
            # loss += 150 * F.kl_div(s_rpn_cls,
            #                 torch.sigmoid(T_ret_dict['rpn_cls'].view(-1)/T),
            #                 reduction='batchmean') * T * T

            # loss += 5 * F.kl_div(F.log_softmax(ret_dict['rpn_reg']/T, dim=1),
            #     F.softmax(T_ret_dict['rpn_reg']/T, dim=1),
            #     reduction='batchmean') * T * T
        ...
        if cfg.RCNN.ENABLED:
            ...
            ######################
            # s_rcnn_cls = torch.sigmoid(ret_dict['rcnn_cls'].view(-1)/T).log()
            # loss += 160 * F.kl_div(s_rcnn_cls,
            #     torch.sigmoid(T_ret_dict['rcnn_cls'].view(-1)/T),
            #     reduction='batchmean') * T * T

            # loss += 5 * F.kl_div(F.log_softmax(ret_dict['rcnn_reg']/T, dim=1),
            #     F.softmax(T_ret_dict['rcnn_reg']/T, dim=1),
            #     reduction='batchmean') * T * T
    ...
    def get_rpn_loss_KD(model, rpn_cls, rpn_cls_label, tb_dict, T):
        ...
        if cfg.RPN.LOSS_CLS == 'DiceLoss':
            ...
            rpn_loss_cls = rpn_cls_loss_func(rpn_cls_flat / T, rpn_cls_target / T, cls_weights)
    ...
    def get_sample_loss(model, sample_cls, sample_idx,rpn_cls_label,tb_dict):####sample_idx: 1x1024   rpn_cls_label 1x16384
        if isinstance(model, nn.DataParallel):
            sample_cls_loss_func = model.module.rpn.backbone_net.MSG_sample_loss_func
        else:
            sample_cls_loss_func = model.rpn.backbone_net.MSG_sample_loss_func
        
        pos = (rpn_cls_label.view(-1)>0).sum(dim=0).float()
        
        # sa_ins_labels = rpn_cls_label.gather(1,idx.long())#####1x1024
        idx_0 = sample_idx[0]
        label_0 = rpn_cls_label.gather(1,idx_0.long())#####1x4096

        # pos_sample = (label_0.view(-1)>0).sum(dim=0).float()
        # dic['SA1'] += pos_sample.item()

        sa_ins_labels = [label_0]
        for j in range(len(sample_idx)-1):
            idx = sample_idx[j+1] ####1x1024
            # pos1 = (sa_ins_labels[j].view(-1)>0).sum(dim=0).float()
            label = sa_ins_labels[j].gather(1,idx.long())

            # pos_sample_temp = (label.view(-1)>0).sum(dim=0).float()
            # dic['SA' +str(j+2)] += pos_sample_temp.item()
            
            sa_ins_labels.append(label)
        sa_ins_loss, ignore = 0, 0

        temp = (sa_ins_labels[0].view(-1)>0).sum(dim=0).float()
        for i in range(len(sample_cls)):
            if len(sample_cls[i]) != 0 and len(sample_idx[i]) !=0:

                sa_ins_pred = sample_cls[i]
          
            else:
                ignore+=1
                continue
            point_cls_pred = sa_ins_pred.view(-1)###1024x1
            
           
            point_cls_label = sa_ins_labels[i].view(-1)#####1024
            # a=(point_cls_labels * (point_cls_labels >= 0).long()).unsqueeze(dim=-1).long()
            positives = (point_cls_label > 0)
            negative_cls_weights = (point_cls_label == 0) * 1.0
            cls_weights = (negative_cls_weights + 1.0 * positives).float()
            pos_normalizer = positives.sum(dim=0).float()
            cls_weights /= torch.clamp(pos_normalizer, min=1.0)
            one_hot_targets = point_cls_pred.new_zeros(*list(point_cls_label.shape), 2)###1024x1  0:background 1:forepoint
            one_hot_targets.scatter_(-1, (point_cls_label * (point_cls_label >= 0).long()).unsqueeze(dim=-1).long(), 1.0)
            one_hot_targets = one_hot_targets[...,1:]

            if i == 0 :
                split = 16
                intevel = point_cls_pred.size(0)//split
                for k in range(split):
                    point_cls_pred_temp = point_cls_pred[k * intevel:(k+1) * intevel]
                    one_hot_targets_temp = one_hot_targets[k * intevel:(k+1) * intevel, :]
                    cls_weights_temp = cls_weights[k * intevel:(k+1) * intevel]
                    point_loss_ins = sample_cls_loss_func(point_cls_pred_temp, one_hot_targets_temp, weights=cls_weights_temp).mean(
                        dim=-1).sum()
                    
                    sa_ins_loss += point_loss_ins 
            else :

                point_loss_ins = sample_cls_loss_func(point_cls_pred, one_hot_targets, weights=cls_weights).mean(dim=-1).sum()
                sa_ins_loss += point_loss_ins
        ...
        if ignore != 4:
            sa_ins_loss = sa_ins_loss / (len(sample_cls) - ignore)
            tb_dict.update({
